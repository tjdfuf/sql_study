...
-- REPLACE : 조회 할 때 바꾼걸로 보여주고 실제 데이터가 바뀌는건 아님.
-- 실제 데이터 변경은 UPDATE로만 가능함.
SELECT 
    REPLACE(sn, ' ', '+') AS sn,
...
-- 상수문자열과의 비교
-- 비교대상에 맞춰서 비교함 ( char = 상수 : char = char )
SELECT 
    REPLACE(char_4, ' ', '+') AS char_4,
    REPLACE(varchar_6, ' ', '+') AS varchar_6
FROM varchar_compare
...
SELECT
    ...
    TO_CHAR(SYSDATE, 'YYYY"년" MM"월" DD"일"') AS "날짜 - 한글포함", -- 한글 쓸때 ""안에 넣어서 받아야됨
...
-- 존재하지 않는 사람 검색시 아무것도 안뜸 : NULL도 하나의 행을 출력하는것임
-- MAX : 최대값 찾는 함수 MIN : 최솟값
SELECT 
--    MAX(emp_nm) -- NULL이 뜸
