...
-- Cartesion Product(Cross Join) : JOIN 조건을 설정하지 않으면 모든 경우의 수를 다 보여준다 (a*b의 갯수만큼)
SELECT 
    *
FROM test_a, test_b
;

-- INNER JOIN(EQUI JOIN의 한 종류) : 동등 조건에 매칭되는 것만 걸러주는 것
SELECT 
    *
FROM test_a, test_b
WHERE test_a.id = test_b.a_id  -- JOIN 조건
;
...
-- # INNER JOIN
-- 1. 2개 이상의 테이블이 공통된 컬럼에 의해 논리적으로 결합되는 조인가법입니다.
-- 2. WHERE절에 사용된 컬럼들이 동등연사자(=)에 의해 조인됩니다.

-- 용인시에 사원의 사원번호, 사원명, 주소, 부서코드, 부서명을 조회하고 싶다.
SELECT 
    A.emp_no
    , A.emp_nm
    , A.addr
...
SELECT 
    ...
    , b.dept_nm
FROM tb_emp A 
INNER JOIN tb_dept B  -- JOIN이라고 쓰면 앞에 INNER가 생략되어서 적혀져 있음
...
-- 5. 공통 컬럼이 N개 이상이면 조인 조건이 N개로 처리됩니다. (N > 1)

-- 사원 테이블과 부서 테이블을 조인
SELECT
    A.emp_no
    , A.emp_nm
    , dept_cd -- NATURAL JOIN한 것 앞에 별칭을 붙이면 안된다.
    , B.dept_nm
FROM tb_emp A
...
SELECT
    A.emp_no
    , A.emp_nm
    , dept_cd -- USING 한 것 앞에 별칭을 붙이면 안된다.
    , B.dept_nm
FROM tb_emp A
